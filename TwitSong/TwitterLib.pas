...
Uses Windows, SysUtils, Classes, StdCtrls, Dialogs, ShellApi,
...
Const RequestTokenURL     = 'https://api.twitter.com/oauth/request_token';
      ...
      RequestTwitURL      = 'https://api.twitter.com/1.1/statuses/update.json';             // TWITTER API 1.1
...
Type TwitterCli = class(TObject)
  private
    ...
    procedure SSLHTTPClientRequestDone(Sender: TObject; RqType: THttpRequest; ErrCode: Word);
...
begin
  TStamp := IntToStr(GetUTCUnixTime);
end;
  ...
  begin
    ...
    on E: Exception do
      begin
        RcvdStream.Destroy; RcvdStream := nil;
        SendStream.Destroy; SendStream := nil;
        SSLHTTPClient.SslContext.Destroy; SSLHTTPClient.SslContext := nil;
        ResultStrings.Text := 'ERROR '+ E.Message;
        LastHttpStatus := SSLHTTPClient.StatusCode;
        FreeAndNil(SSLHTTPClient);
        InternalCallback;
...
procedure TwitterCli.SSLHTTPClientRequestDone(Sender: TObject; RqType: THttpRequest; ErrCode: Word);
Var utf8LoadingResult: byte;
    Err: string;
Label CleanUp;
begin
  ...
  begin
    ...
    if DebugMode then if Assigned(DebugMemo) then with TMemo(DebugMemo) do
     Lines.Add('result='+ResultStrings.Text);
      ...
      if (ErrCode <> 0) or (SSLHTTPClient.StatusCode <> 200) then Goto CleanUp;
    end
    else Exit;

CleanUp:

  SSLHTTPClient.RcvdStream.Destroy; SSLHTTPClient.RcvdStream := nil;
  SSLHTTPClient.SslContext.Destroy; SSLHTTPClient.SslContext := nil;
...
begin
  frmRequest.browser.Navigate('https://twitter.com/oauth/authorize?oauth_token='+OAuthToken);
  frmRequest.Show;
end;
...
begin
  frmMain.Memo1.Lines.Add('TwitLib http codes : ' + IntToStr(LastHttpStatus));

  if (LastHttpStatus = 401) then MessageBox(0,'Authentication credentials were missing or incorrect.' + Char(13) + 'This error may occured by incorrect date/time settings.', '401 Error', mb_ok + mb_iconwarning);
